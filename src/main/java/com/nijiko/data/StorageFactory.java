...
public class StorageFactory {
    public static final IStorage createInstance(String world, Configuration config) throws Exception
    {
        config.load();
        ...
        case YAML:
            ...
            if(!userFile.isFile()) throw new Exception("User config for world "+ world +" is not a file.");
            if(!userFile.canRead()) throw new Exception("User config for world "+ world +" cannot be read.");
            if(!userFile.canWrite()) throw new Exception("User config for world "+ world +" cannot be written to.");
            if(!userFile.exists()) userFile.createNewFile();
            File groupFile = new File(worldString + "groups.yml");
            if(!groupFile.isFile()) throw new Exception("Group config for world "+ world +" is not a file.");
            if(!groupFile.canRead()) throw new Exception("Group config for world "+ world +" cannot be read.");
            if(!groupFile.canWrite()) throw new Exception("Group config for world "+ world +" cannot be written to.");
