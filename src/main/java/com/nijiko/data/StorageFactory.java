...
public class StorageFactory {
    ...
    public final static UserStorage getUserStorage(String world) throws IOException
    {
        ...
        if(userWorldtype==null||userWorldtype.isEmpty()) userWorldtype = typename;
        ...
        case YAML:
            ...
            if(!userFile.isFile()) throw new IOException("User config for world "+ userWorld +" is not a file.");
            if(!userFile.canRead()) throw new IOException("User config for world "+ userWorld +" cannot be read.");
            if(!userFile.canWrite()) throw new IOException("User config for world "+ userWorld +" cannot be written to.");
    ...
    public final static GroupStorage getGroupStorage(String world) throws IOException
    {
        ...
        if(groupWorld==null) groupWorld = world;
        parent = null;
        String typename = config.getString("permissions.storage.type",StorageType.YAML.toString());
        String groupWorldtype = config.getString("permissions.storage.group.worldtype." + groupWorld);
        if(groupWorldtype==null||groupWorldtype.isEmpty()) groupWorldtype = typename;
        ...
        case YAML:
            ...
            if(!userFile.isFile()) throw new IOException("Group config for world "+ groupWorld +" is not a file.");
            if(!userFile.canRead()) throw new IOException("Group config for world "+ groupWorld +" cannot be read.");
            if(!userFile.canWrite()) throw new IOException("Group config for world "+ groupWorld +" cannot be written to.");
            if(!userFile.exists()) userFile.createNewFile();
