...
public class YamlGroupStorage implements GroupStorage{
    ...
    public Set<GroupWorld> getParents(String name) {
        rwl.readLock().lock();
        Set<String> rawParents = new HashSet<String>(groupConfig.getStringList("groups."+name+".inheritance" , null));
        rwl.readLock().unlock();
        Set<GroupWorld> parents = new HashSet<GroupWorld>(rawParents.size());
        for(String raw : rawParents)
        {
            String[] split = raw.split(",",2); //Split into at most 2 parts ("world,blah" -> "world", "blah")("blah" -> "blah")
    ...
    public void addParent(String name, String groupWorld, String groupName) {
        ...
        else permissions.add(groupWorld+","+groupName);
    ...
    public void removeParent(String name, String groupWorld, String groupName) {
        ...
        else permissions.remove(groupWorld+","+groupName);
        groupConfig.setProperty("groups."+name+".inheritance", new LinkedList<String>(permissions)); 
        modified = true;
        save();
        rwl.writeLock().unlock();
    ...
    public Set<String> getGroups() {
        rwl.readLock().lock();
        Set<String> users = new LinkedHashSet<String>(groupConfig.getKeys("groups"));
        rwl.readLock().unlock();
        return users;
    }
    ...
    public void forceSave() {
        ...
        if(rwl.isWriteLockedByCurrentThread())writeLocked = true;
        else rwl.writeLock().lock();
        if(modified) groupConfig.save();
        groupConfig.load();
        modified = false;
        if(!writeLocked) rwl.writeLock().unlock();        
    }
    ...
    public void save() {
        ...
        if(saveOff) return;
        if(!writeLocked)rwl.readLock().unlock();
        forceSave();
    }
